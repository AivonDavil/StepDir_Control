...
void Motor::Setup_motor_test(){
  ...
  int N = 100* pow(2,MOTOR_STEP); //считаем сколько шагов нужно для полного оборота 
  for( int i = 0; i<N; i++ ){
    this->runONEstep();
  }
  digitalWrite( EN_PIN, HIGH );
...
void Motor::Change_step( int step ){
...
void Motor::Change_dir(){
  ...
  digitalWrite( DIR_PIN, !(digitalRead( DIR_PIN )) ); // меняем направление на противоположное 

}

void Motor::Change_step_time(int step_time){
